...
do
    local ENCOUNTER_ID = 2609
    local EVENT_DEVOUR = 868
    local EVENT_SPELL_ID = 373686
    local SHIELD_BASE_AMOUNT = 2490819.8
    ...
    local Mod = ExBoss.Modules.Boss.MelidrussaShieldBar or {}
    ExBoss.Modules.Boss.MelidrussaShieldBar = Mod
    ...
    if ExwindTools and type(ExwindTools.RegisterEvent) == "function" then
        ExwindTools:RegisterEvent("EXBOSS_FIXED_AI_EVENT_SCHEDULED", "ExBoss_Melidrussa_ShieldWatch_Scheduled", OnFixedAIEventScheduled)
        ExwindTools:RegisterEvent("ENCOUNTER_START", "ExBoss_Melidrussa_ShieldWatch_Reset", function(_, encounterID)
        ...
        ExwindTools:RegisterEvent("ENCOUNTER_END", "ExBoss_Melidrussa_ShieldWatch_End", function(_, encounterID)
            if encounterID == nil or tonumber(encounterID) == ENCOUNTER_ID or IsRaviActive() then
                HideShieldBar()
            end
        end)
        ExwindTools:RegisterEvent("PLAYER_ENTERING_WORLD", "ExBoss_Melidrussa_ShieldWatch_PEW", function()
            if FORCE_TARGET_TEST == true then
                StartShieldWatch()
            else
                HideShieldBar()
...
R:Register({
    ...
    healthThresholds = {
        { threshold = 66, preset = "phase_transition" },
        { threshold = 33, preset = "phase_transition" },
    },
    phaseAlerts = {},
    vulnerability = {},
    extras = {},
