...
-- 页面只保留真实配置字段；卡片内 Grid 坐标从各自 Body 起算。
local FILL_MODE_ITEMS = {
...
local CARD_GUI = {
    ...
    description = L["屏幕中央显示圆环进度"] or "屏幕中央显示圆环进度",
    cards = {
        { id = "general", title = L["功能"] or "功能", content = { kind = "grid", items = {
            { key = "enabled", type = "checkbox", x = 3, y = 1, w = 46, h = 6, label = L["启用"] or "启用" },
        } } },
        { id = "anchor", title = L["锚点设置"] or "锚点设置", content = { kind = "composite", component = "anchorgroup", key = "anchor", opts = ANCHOR_OPTS } },
        { id = "ring", title = L["圆环外观"] or "圆环外观", content = { kind = "grid", items = {
            { key = "castFillMode", type = "dropdown", x = 4, y = 1, w = 44, h = 6, label = L["施法填充方式"] or "施法填充方式", items = FILL_MODE_ITEMS, labelPos = "top" },
            { key = "channelFillMode", type = "dropdown", x = 52, y = 1, w = 44, h = 6, label = L["引导填充方式"] or "引导填充方式", items = FILL_MODE_ITEMS, labelPos = "top" },
            { key = "style", type = "dropdown", x = 103, y = 1, w = 44, h = 6, label = L["圆环样式"] or "圆环样式", items = {
                { L["细环 1"] or "细环 1", "thin1" },
                { L["细环 2"] or "细环 2", "thin2" },
                { L["标准环"] or "标准环", "classic" },
            }, labelPos = "top" },
            { key = "size", type = "slider", x = 4, y = 14, w = 44, h = 6, label = L["圆环尺寸"] or "圆环尺寸", min = 20, max = 360, step = 2 },
            { key = "ringColor", type = "color", x = 52, y = 14, w = 44, h = 6, label = L["颜色"] or "颜色" },
            { key = "alpha", type = "slider", x = 103, y = 14, w = 44, h = 6, label = L["透明度"] or "透明度", min = 0.1, max = 1, step = 0.05 },
            { key = "testCast", type = "button", x = 151, y = 14, w = 23, h = 6, label = L["测试施法"] or "测试施法", func = function() GetRingProgress():ShowTestCast() end },
            { key = "testChannel", type = "button", x = 176, y = 14, w = 23, h = 6, label = L["测试引导"] or "测试引导", func = function() GetRingProgress():ShowTestChannel() end },
        } } },
        { id = "background", title = L["背景圆环"] or "背景圆环", content = { kind = "grid", items = {
            { key = "bgEnabled", type = "checkbox", x = 1, y = 1, w = 48, h = 6, label = L["启用背景圆环"] or "启用背景圆环" },
            { key = "bgColor", type = "color", x = 4, y = 11, w = 44, h = 6, label = L["背景颜色"] or "背景颜色" },
            { key = "bgAlpha", type = "slider", x = 52, y = 11, w = 44, h = 6, label = L["背景透明度"] or "背景透明度", min = 0.05, max = 1, step = 0.05 },
        } } },
        { id = "name", title = L["法术名称"] or "法术名称", content = { kind = "composite", component = "fontgroup", key = "font_spell" } },
        { id = "time", title = L["时间文本"] or "时间文本", content = { kind = "composite", component = "fontgroup", key = "font_timer" } },
    },
}
